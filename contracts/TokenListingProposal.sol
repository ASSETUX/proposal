...
contract TokenListingProposal is Initializable, UUPSUpgradeable, OwnableUpgradeable, AccessControlUpgradeable {
    ...
    IERC20Upgradeable public asxAddress = IERC20Upgradeable(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48); // USDC for test
    address public incentiveTokenAddress;
    uint256 public destributionPeriod;
    uint256 public proposalDeadline; // deadline, when Proposal will be Removed
    uint256 public proposalCreationTimestamp;
    uint256 public hostAmount; // amount from which user is considered a host

    uint256 approveAmount;
    uint256 removeAmount;

    uint256 public totalAsxOnProposal;
    uint256 public totalIncentiveTokenOnProposal;
    ...
    event NeedApproveProposal(uint256 totalAsxOnProposal);
    ...
    function initialize(
        ...
        require(IERC20Upgradeable(_incentiveTokenAddress).allowance(msg.sender, address(this)) < _incentiveTokenAmount, "No insentive token allowance");
        ...
        totalIncentiveTokenOnProposal += _incentiveTokenAmount;
    ...
    function stakeOnProposal(uint256 _amountToStake, uint256 _lockPeriod) external {
        ...
        require(IERC20Upgradeable(asxAddress).allowance(msg.sender, address(this)) < _amountToStake, "No ASX allowance");
        ...
        totalAsxOnProposal += amountWithTimeLock;
        ...
        if (totalAsxOnProposal >= approveAmount) {
            emit CanApproveProposal(totalAsxOnProposal);
        }
    }

    function claimRewards() external isApproved {
        uint256 userReward = calcRewards();
        totalIncentiveTokenOnProposal -= userReward;
    ...
    function calcRewards() public returns(uint256) {
        uint256 generelRewardPerSecond = totalIncentiveTokenOnProposal / destributionPeriod;
        uint256 userReward;
        for (uint256 i = 0; i < accountsStakingInfo[msg.sender].allStakes.length; i++) {
            uint256 stakeShareOfTotalStake = accountsStakingInfo[msg.sender].allStakes[i].stakeAmountWithTimeLock / totalAsxOnProposal;
    ...
    function withdrawWhenApproved(uint256 _amount) external isApproved {
        ...
        totalAsxOnProposal -= amountToWithdrawWithTimeLock;
        totalAsxOnProposal += newStakeAmount;
        asxAddress.transfer(msg.sender, _amount);

        if (totalAsxOnProposal < removeAmount) {
            state = State.Removed;
            emit NeedApproveProposal(totalAsxOnProposal);
        }
    }
    ...
    function withdrawWhenRemoved() external isRemoved {
        ...
        totalAsxOnProposal -= accountsStakingInfo[msg.sender].totalStakeAmountWithTimeLock = 0;
    ...
    function withdrawIncentiveTokenWhenRemoved() external isRemoved {
        ...
        totalIncentiveTokenOnProposal -= amountToWithdraw;
    ...
    function burnAsx(uint256 _amount) external isApproved {
        ...
        totalAsxOnProposal -= amountToBurnWithTimeLock;
        totalAsxOnProposal += newStakeAmount;
    ...
    function approveProposal() external onlyRole(ADMIN) {
        require(totalAsxOnProposal >= approveAmount);
